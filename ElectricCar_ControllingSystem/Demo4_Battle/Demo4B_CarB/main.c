...
void USART2_IRQHandler(){
    if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)
    {
				...
				if (strBuffer[strPos-8] == 'C' && strBuffer[strPos-7] == 'R' && strBuffer[strPos-6] == 'D')							//Red
					{
				...
				if (strBuffer[strPos-8] == 'C' && strBuffer[strPos-7] == 'P' && strBuffer[strPos-6] == 'K')							//Pink
					{
...
void tune_Motor(int offset, float Kp,float Ki,float Kd){
		float PID, left_Motor, Right_Motor;
			...
			left_Motor = offset+PID ;										//CarA is -PID, CarB is +PID
...
void progress_Table()
{
			...
			{		
					...
					predict_Ball();
					cal_ErrorFront(BallXY);
					tune_Motor(1000,Kp_speed,Ki_speed,Kd_speed);
					if (CarFrontXY[0]<LeftLimit)
					{
							stop_Motor();
							reverse_Motor();
							stop_Motor();
...
int main(void) 
{
	...
	SysTick_init();	
	USART2_init();
	USARTSend(USART2, send_WifiTesting1,sizeof(send_WifiTesting1));
	delayMs(100);
